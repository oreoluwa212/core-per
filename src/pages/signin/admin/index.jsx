    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            if (response.data.msg === "Admin signed in successfully") {
                ...
                setSubmitError(response.data.msg || "An error occurred. Please try again.");
                toast.error(response.data.msg || "An error occurred. Please try again."); // Show error toast
            }
        } catch (error) {
        ...
        <div>
            ...
            <div className='lg:grid-cols-2 h-screen grid grid-cols-1 bg-black text-white'>
                <div className='lg:flex h-full hidden'>
                    <img src={heroImg2} alt="Signin Image" className='object-cover w-full max-h-screen' />
                </div>
                        ...
                        <div className="w-full flex justify-center items-center">
                            <div className="my-auto w-full rounded-[16px] p-6">
                                <div className="mb-4 flex justify-center rounded-[62px] border">
                                    <Link
                                        to="/signup/admin"
                                        className={`w-1/2 rounded-[61px] px-4 py-2 text-center ${isSignIn ? "" : "bg-gray-400"} transition-all duration-300`}
                                ...
                                <form onSubmit={handleSubmit(onSubmit)}>
                                    <div className="mb-4">
                                        <label htmlFor="email" className="mb-2 block text-[16px] text-secondary-120">
                                            Email
                                        </label>
                                        <input
                                            id="email"
                                    ...
                                    <div className="relative mb-4">
                                        <label htmlFor="password" className="mb-2 block text-[16px] text-secondary-120">
                                            Password
                                        </label>
                                        <input
                                            id="password"
                                    ...
                                    <div className="flex justify-between">
                                        <Link to="#" className="text-[14px] text-secondary-120 underline hover:text-gray-200">
                                            Forgot Password?
                                        </Link>
                                    </div>
                                    <div className="mt-6">
                                        <button
                                            type="submit"
                                            className={`w-full rounded-[10px] bg-gray-700 border-white py-4 text-white flex items-center justify-center ${loading ? "opacity-50" : "hover:bg-primary-110"}`}
                                            disabled={loading}
                                        >
                                            {loading ? <ClipLoader size={20} color="#ffffff" /> : "Sign In"} {/* Show spinner when loading */}
                                        </button>
                                    </div>
                                    {submitError && (
